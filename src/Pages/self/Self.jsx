...
import {
  ...
  addImage,
  deleteImage,
} from "../../store/actions/self-action";
...
const SelfPage = () => {
  ...
  const data = useSelector((state) => state.self.data);
    ...
    setUpdatedData((prevData) =>
      prevData.map((item) =>
        item.id === id
          ? {
              ...item,
              SelfWashImages: [...(item.SelfWashImages || []), newData],
            }
          : item
      )
    );
    ...
    setUpdatedData((prevData) =>
      prevData.map((item) =>
        item.id === serviceId
          ? {
              ...item,
              SelfWashImages: item.SelfWashImages.filter(
